...
from lib.train.actors import SALTTrackActor
# for import modules
import importlib
...
def run(settings):
    settings.description = 'Training script for salttrack'
    ...
    # Create network
    if settings.script_name == "salttrack":
        net = build_salttrack(cfg)
    else:
    ...
    if settings.script_name == "salttrack":
        ...
        actor = SALTTrackActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
