...
class Got10k(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None,
        ...
        self.nlp_label_dir = os.path.join(current_dir,"../../../resource/vl_datasets_label/got_10k/nlp")
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        # nlp_label_file = os.path.join(self.nlp_label_dir,self.sequence_list[seq_id]+".txt" )
        ...
        if random.random() >  0.5:
            anno_frames["nlp"] = [obj_meta["object_class_name"] + " " + obj_meta["motion_class"]]
            class_len = len(obj_meta["object_class_name"].split(" "))
            subject_mask_infor = []
            ...
            anno_frames["nlp"] = [obj_meta["major_class"] + " " + obj_meta["motion_class"]]

            class_len = len(obj_meta["major_class"].split(" "))
            subject_mask_infor = []
            for index in range(class_len):
                subject_mask_infor.append(index)
            nlp_with_mask = anno_frames["nlp"][0] + "+" + ",".join(map(str, subject_mask_infor))
